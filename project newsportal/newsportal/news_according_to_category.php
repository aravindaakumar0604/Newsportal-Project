...
?>
			<div class="latest_posts">
				<a href="view_individual_page.php?title=<?php echo $row['title'];?>" value="<?php echo $row['title'];?>" name="<?php echo $row['title'];?>"><img src="images/<?php echo $row['thumbnails'];?>" alt="Image here" width="50" height="50"><br/>
...
<?php 
	...
	while($row = mysqli_fetch_assoc($result)){
?>
		<a href="view_individual_page.php?title=<?php echo $row['title'];?>" value="<?php echo $row['title'];?>" name="<?php echo $row['title'];?>"><font class="breaking">breaking news :<?php echo $row['title'];?></font>&nbsp;&nbsp;
...
<?php 
	...
	$sql = "select * from news where category='$category' limit $page1,3";
	$result = mysqli_query($conn,$sql);
	while($row = mysqli_fetch_assoc($result)){
?>
 ...
 <?php
 	...
 	$run = mysqli_query($conn,"select * from news where category='$category'");
 	...
 	<ul class="page">
 		<li class="pagination">
 		<a href="news_according_to_category.php?category_name=<?php echo $category;?>&page=<?php echo $b;?>" style="color:blue;"><?php echo $b; ?></a></li>
 	</ul>	
 	<?php 
 		}
		?>
